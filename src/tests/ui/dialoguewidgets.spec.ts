...
test('InputItemsDialog', () => {
    let dialog = new InputItemsDialog({ title: '', items: ["one", "two"], current: 1, editable: true})
    expect(dialog.input.type).toEqual('text')
    ...
    dialog = new InputItemsDialog({ title: '', items: ["one", "two"], current: 0, editable: true})
    expect(dialog.getValue()).toEqual("one")
})
...
describe('ValidatedInput', () => {
    test('construction no postprocess', () => {
        ...
        const validated = new ValidatingInput(input, 
            (value) => value == 'valid'
        )
