import { InputBooleanDialog, InputDateDialog, InputJSONDialog, InputNumberDialog, InputPasswordDialog, InputTextDialog } from "../../ui/dialogwidgets";
...
describe('createMetaInput', () => {
    ...
    test('date MetaInput', () => {
        ...
        const input = createMetaInput({'type': 'string', format: 'date'}, initial, '')
    ...
    test('password MetaInput', () => {
        ...
        const input = createMetaInput({'type': 'string', format: 'password'}, initial, '')
    ...
    test('unknown string format MetaInput', () => {
        const initial = 'a secret'
        const input = createMetaInput({'type': 'string', format: 'byte'}, initial, '')
        expect(input).toBeInstanceOf(InputTextDialog)
        expect(input.getValue()).toEqual(initial)
    })
...
describe('createValidatedInput', () => {
    ...
    test('date postProcessing', () => {
        ...
        const validated = createValidatedInput({'type': 'string', 'format': 'date'}, intial, '')
    ...
    test('additional schema validation', () => {
        const intial = 5
        const validated = createValidatedInput({'type': 'integer', 'multipleOf': 5}, intial, '')
